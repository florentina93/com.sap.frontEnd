sap.ui
		.controller(
				"sap.ui.demo.myFiori.view.Detail",
				{
					...
					onRouteMatched : function(oEvent) {
						...
						this.getView().byId("nametext").setText(
								oModel.getData().name);
						this.getView().byId("hometowntext").setText(
								oModel.getData().hometown.name);
						this.getView().byId("firstnametext").setText(
								oModel.getData().first_name);
						this.getView().byId("idtext").setText(
								oModel.getData().id);
						this.getView().byId("birthdaytext").setText(
								oModel.getData().birthday);
						this.getView().byId("gendertext").setText(
								oModel.getData().gender);
					...
					bindView : function(sProductPath) {
						...
						if (!oView.getModel().getData(sProductPath)) {
							...
							// Check that the product specified actually was
							// found.
							oView
									.getElementBinding()
									.attachEventOnce(
													...
													.proxy(
															function() {
																...
																if (!oData) {
																	...
																	this
																			.fireDetailChanged(sProductPath);
																}
															}, this));
				...
				// // //oModel.loadData("json/plants.json", false);
				// //
				// oModel.loadData("http://localhost:8081/com.sap.crawler/getdata",
				// "", false);
				// // sap.ui.getCore().setModel(oModel,"MAIN");
				// // },
